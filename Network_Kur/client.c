...
void processCommands()
{
	...
	while (strcmp(currentCommand.commandName, "quit") != 0)
	{
		...
		if (strcmp(currentCommand.commandName, "showHistory") == 0)
		{
			showHistoryCommand();
			ok = 1;
		}
...
void processStatusCommand()
{
	...
	if (currentCommand.nrParameters != 0)
	{
		printf("[From Status] Error: Invalid number Of parameters!\n");
		return;
	}
	/* I send the command to the server to be processed. */
	if (sendCommand(sd, currentCommand, key) == -1)
	{
		printf("[From Status] Error while sending the command!\n");
		return;
	}
	...
	if (read(sd, &c, sizeof(char)) < 0)
	{
		printf("[From Status] Error reading junk character!\n");
		return;
	}
...
void processCommitCommand()
{
	...
	if (sendCommand(sd, currentCommand, key) == -1)
	{
		printf("[From Commit] Error while sending the command!\n");
		return;
	}
	...
	if (result == 0)
	{
		printf("[From Commit] The work files couldn't be sent!\n");
		return;
	}
	else
	{
		printf("[From Commit] The work files were sent!\n");
	}
	...
	{
		printf("[From Commit] Error: Couldn't read commit result!\n");
		return;
	}
	/* Convert from network format to host format. */
	result = ntohl(result);
	...
	if (result == 0)
	{
		printf("[From Commit] The work files were received!\n");
	}
	else
	{
		printf("[From Commit] The work files weren't received!\n");
	}
}
